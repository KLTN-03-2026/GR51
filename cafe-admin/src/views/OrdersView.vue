    ...
    <div class="toolbar">
      <div class="toolbar-actions">
        ...
        <select v-model="filters.trang_thai_don" class="filter-select" @change="load">
          <option value="">Trạng thái</option><option value="dang_pha">Đang pha</option><option value="hoan_thanh">Hoàn thành</option>
        </select>
        <select v-model="filters.trang_thai_thanh_toan" class="filter-select" @change="load">
          <option value="">Thanh toán</option><option value="da_thanh_toan">Đã TT</option><option value="chua_thanh_toan">Chưa TT</option>
        </select>
      </div>
    </div>
    ...
    <table class="data-table">
      ...
      <tbody>
        <tr v-for="d in orders" :key="d.ma_don_hang">
          ...
          <td><span :class="d.trang_thai_thanh_toan==='da_thanh_toan'?'badge badge-success':'badge badge-error'">{{ d.trang_thai_thanh_toan==='da_thanh_toan'?'Đã TT':'Chưa' }}</span></td>
          <td><span class="badge" :class="{'badge-warning':d.trang_thai_don==='dang_pha','badge-success':d.trang_thai_don==='hoan_thanh','badge-info':d.trang_thai_don==='da_pha_che'}">{{ {dang_pha:'Đang pha',da_pha_che:'Đã pha',hoan_thanh:'Hoàn thành'}[d.trang_thai_don]||d.trang_thai_don }}</span></td>
          <td>{{ d.thoi_gian }}</td>
